    ...
    <>
      ...
      <div className="modal">
        <header className="modal-head">
          <h3>Stage gate · {stage.name} → {next.name}</h3>
        ...
        <ul className="gates modal-gates">
          {stage.exitGates.map((g) => {
            ...
            return (
              <li key={g.id} className={ev ? "met" : "unmet"}>
                ...
                {ev ? (
                  <span className="gate-ev mono">{ev.at} · {ev.via}</span>
                ) : (
                  <span className="gate-ev mono missing">missing</span>
                )}
              </li>
            );
          })}
        ...
        <footer className="modal-foot">
          ...
          {allMet ? (
            <button className="btn on" onClick={() => move(false)} disabled={busy}>
              Advance — gates clear
            </button>
          ) : (
            <>
              <span className="modal-warn">{unmet.length} gate(s) unmet. File the missing updates, or force it:</span>
              <button className="btn danger" onClick={() => move(true)} disabled={busy}>
                ⚑ Force move with override flag
              </button>
            </>
          )}
        </footer>
